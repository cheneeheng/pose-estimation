...
IMAGE_NAME="alphapose-user:cuda11.7.1-cudnn8-devel-ubuntu20.04"
DATA_PATH=$2
CODE_PATH=$3
...
docker run -it --rm --gpus=all \
    ...
    --env PYTHONPATH=/usr/local/lib:/code/pose-estimation:/code/pose-estimation/alphapose \
    --device-cgroup-rule "c 81:* rmw" \
    --device-cgroup-rule "c 189:* rmw" \
    --mount type=bind,source=/tmp/.X11-unix,target=/tmp/.X11-unix \
    --mount type=bind,source=${CODE_PATH},target=/code/pose-estimation \
    --mount type=bind,source=${DATA_PATH},target=/data/alphapose \
