...
class App():
    def __init__(self) -> None:
        ...
        self.model_pos = YOLO("yolov8s-pose.pt")
    ...
    def infer_pose_and_track(self, color_image, depth_image):
        if not hasattr(self.model_pos.predictor, 'trackers'):
            from ultralytics.tracker import register_tracker
            register_tracker(self.model_pos, True)
        results = self.model_pos(
            source=color_image,
            imgsz=480,
            classes=0,
            max_det=10,
        ...
        pose_image = np.zeros_like(color_image)
        skel_added = False
        result = results[0]
        for i in range(result.keypoints.data.shape[0]):
            if result.boxes.data.numel() == 0:
                continue
            if result.boxes.id is None:
                continue
            pose_result = result.keypoints.data[i].cpu().detach().numpy()
            ...
            plot_args = dict(img=pose_image,
                             boxes=True)
            pose_image = result.plot(**plot_args)
            skel_added = True
